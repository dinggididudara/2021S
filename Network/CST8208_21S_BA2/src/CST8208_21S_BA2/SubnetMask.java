...
public class SubnetMask {
	...
	public String subnetMaskBinary(String subnetArrElement1, int subnetInt) { //binary from subnet prefix
		...
		subnetDecimalCalculate(subnetInt);
		
		return b;
	}
	
	public String subnetDecimalCalculate(int subnetInt) {
		...
		subnetMaskFull = subnetSepArr[0] + "." + subnetSepArr[1] + "."  + subnetSepArr[2] + "." + subnetSepArr[3];
		subnetDecimalString = subnetSepString[0] + "." + subnetSepString[1] + "." + subnetSepString[2] + "." + subnetSepString[3]; //in binary
		wildcardCalculate(subnetInt);
		return subnetDecimalString;
	}
	
	public String wildcardCalculate(int subnetInt) {
		...
		for(int x=0;x<subnetInt;x++) {
			c = c.append('0');
		}
		for(int y=subnetInt;y<32;y++) {
			c = c.append('1');
		}
		
		d=c.toString();
		
		d=d.substring(0,8) + "." + d.substring(8,16) + "." + d.substring(16,24) + "." + d.substring(24,32);
		return d;
	}
